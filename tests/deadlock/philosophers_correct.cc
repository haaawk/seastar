...
seastar::future<> philosopher(int id, seastar::semaphore& left, seastar::semaphore& right) {
    return seastar::sleep(std::chrono::seconds(id)).then([&left, &right, id] {
        std::cerr << "Philosopher " << id << " waits on " << (&left) << "... waiters:" << left.waiters() << std::endl;
        return left.wait(1);
    }).then([&right] {
        return right.wait(1);
    });
...
int main(int argc, char** argv) {
    ...
    app.run(argc, argv, [] {
        return seastar::do_with(std::vector<seastar::semaphore>(), std::vector<seastar::future<>>(), [](auto& chopsticks, auto& philosophers) {
            for (int i = 0; i < philosopher_cnt; i++) {
                chopsticks.emplace_back(1);
            }

            for (int i = 0; i < philosopher_cnt - 1; i++) {
                auto fut = eat_in_loop(i, chopsticks[i], chopsticks[i + 1]);
                philosophers.emplace_back(std::move(fut));
            }
            // This is reversed - first right chopstick, then left.
            auto fut = eat_in_loop(philosopher_cnt - 1, chopsticks.front(), chopsticks.back());
            philosophers.emplace_back(std::move(fut));
